...
| `render.py` | Deterministic software renderer: ray-marched DEM, rocks, rover body/wheels, sun-direction shading, cast shadows, dust haze, regolith texture, ambient occlusion, wheel ruts, filmic tone map; HUD or clean caption; five cameras (`chase`, `rover_cam`, `overview`, `orbit`, `cinematic`); optional supersampling; contact sheets. | rover, perception, Pillow |
